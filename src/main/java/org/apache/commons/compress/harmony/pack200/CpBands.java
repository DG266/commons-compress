...
public class CpBands extends BandSet {
    ...
    private static final String LOG_TEXT = "Writing ";
    private static final String WROTE_LOG_TEXT = "Wrote ";
    ...
    private void writeCpClass(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log(LOG_TEXT + cp_Class.size() + " Class entries...");
        ...
        PackingUtils.log(WROTE_LOG_TEXT + encodedBand.length + " bytes from cpClass[" + cpClass.length + "]");
    }

    private void writeCpDescr(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log(LOG_TEXT + cp_Descr.size() + " Descriptor entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(WROTE_LOG_TEXT + encodedBand.length + " bytes from cp_Descr_Name[" + cpDescrName.length + "]");

        encodedBand = encodeBandInt("cp_Descr_Type", cpDescrType, Codec.UDELTA5);
        out.write(encodedBand);
        PackingUtils.log(WROTE_LOG_TEXT + encodedBand.length + " bytes from cp_Descr_Type[" + cpDescrType.length + "]");
    }

    private void writeCpDouble(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log(LOG_TEXT + cp_Double.size() + " Double entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(WROTE_LOG_TEXT + encodedBand.length + " bytes from cp_Double_hi[" + highBits.length + "]");

        encodedBand = encodeBandInt("cp_Double_lo", loBits, Codec.DELTA5);
        out.write(encodedBand);
        PackingUtils.log(WROTE_LOG_TEXT + encodedBand.length + " bytes from cp_Double_lo[" + loBits.length + "]");
    }

    private void writeCpFloat(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log(LOG_TEXT + cp_Float.size() + " Float entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(WROTE_LOG_TEXT + encodedBand.length + " bytes from cp_Float[" + cpFloat.length + "]");
    }

    private void writeCpInt(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log(LOG_TEXT + cp_Int.size() + " Integer entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(WROTE_LOG_TEXT + encodedBand.length + " bytes from cp_Int[" + cpInt.length + "]");
    }

    private void writeCpLong(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log(LOG_TEXT + cp_Long.size() + " Long entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(WROTE_LOG_TEXT + encodedBand.length + " bytes from cp_Long_hi[" + highBits.length + "]");

        encodedBand = encodeBandInt("cp_Long_lo", loBits, Codec.DELTA5);
        out.write(encodedBand);
        PackingUtils.log(WROTE_LOG_TEXT + encodedBand.length + " bytes from cp_Long_lo[" + loBits.length + "]");
    }

    private void writeCpMethodOrField(final Set<CPMethodOrField> cp, final OutputStream out, final String name)
        throws IOException, Pack200Exception {
        PackingUtils.log(LOG_TEXT + cp.size() + " Method and Field entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(
                WROTE_LOG_TEXT + encodedBand.length + " bytes from " + name + "_class[" + cp_methodOrField_class.length + "]");

        encodedBand = encodeBandInt(name + "_desc", cp_methodOrField_desc, Codec.UDELTA5);
        out.write(encodedBand);
        PackingUtils
            .log(WROTE_LOG_TEXT + encodedBand.length + " bytes from " + name + "_desc[" + cp_methodOrField_desc.length + "]");
    }

    private void writeCpSignature(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log(LOG_TEXT + cp_Signature.size() + " Signature entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(WROTE_LOG_TEXT + encodedBand.length + " bytes from cpSignatureForm[" + cpSignatureForm.length + "]");

        encodedBand = encodeBandInt("cpSignatureClasses", cpSignatureClasses, Codec.UDELTA5);
        out.write(encodedBand);
        PackingUtils
            .log(WROTE_LOG_TEXT + encodedBand.length + " bytes from cpSignatureClasses[" + cpSignatureClasses.length + "]");
    }

    private void writeCpString(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log(LOG_TEXT + cp_String.size() + " String entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(WROTE_LOG_TEXT + encodedBand.length + " bytes from cpString[" + cpString.length + "]");
    }

    private void writeCpUtf8(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log(LOG_TEXT + cp_Utf8.size() + " UTF8 entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(WROTE_LOG_TEXT + encodedBand.length + " bytes from cpUtf8Prefix[" + cpUtf8Prefix.length + "]");

        encodedBand = encodeBandInt("cpUtf8Suffix", cpUtf8Suffix, Codec.UNSIGNED5);
        out.write(encodedBand);
        PackingUtils.log(WROTE_LOG_TEXT + encodedBand.length + " bytes from cpUtf8Suffix[" + cpUtf8Suffix.length + "]");

        encodedBand = encodeBandInt("cpUtf8Chars", cpUtf8Chars, Codec.CHAR3);
        out.write(encodedBand);
        PackingUtils.log(WROTE_LOG_TEXT + encodedBand.length + " bytes from cpUtf8Chars[" + cpUtf8Chars.length + "]");
